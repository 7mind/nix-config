...
let
  ...
  trunk = cfg.trunk;
  linphoneFallback = cfg.linphoneFallback;
  linphoneFallbackEndpoint = "linphone-fallback";
  linphoneFallbackTransport =
    if linphoneFallback.transport == "tls" then outboundTlsTransportName else cfg.udpTransport;
  ...
  outboundTlsTransportName = "transport-outbound-tls";
  trunkNeedsTlsTransport = trunk.enable && trunk.transport == "tls";
  linphoneFallbackNeedsTlsTransport = linphoneFallback.enable && linphoneFallback.transport == "tls";
  needsOutboundTlsTransport = trunkNeedsTlsTransport || linphoneFallbackNeedsTlsTransport;

  outboundTlsTransport = optionalString needsOutboundTlsTransport ''
    [${outboundTlsTransportName}]
    type=transport
    protocol=tls
    bind=0.0.0.0:${toString trunk.tlsSourcePort}
    method=tlsv1_2
    verify_server=yes
    ca_list_file=${pkgs.cacert}/etc/ssl/certs/ca-bundle.crt
    ${natBlockWithLocalNets}
  '';

  trunkTransportLine = optionalString trunkNeedsTlsTransport "\ntransport=${outboundTlsTransportName}";
  ...
  linphoneFallbackSections = optionalString linphoneFallback.enable ''
    [${linphoneFallbackEndpoint}]
    type=endpoint
    context=${linphoneFallback.inboundContext}
    ${codecBlock cfg.codecs}
    outbound_auth=${linphoneFallbackEndpoint}
    aors=${linphoneFallbackEndpoint}
    transport=${linphoneFallbackTransport}
    from_user=${linphoneFallback.username}
    from_domain=${linphoneFallback.domain}
    direct_media=no
    force_rport=yes
    rewrite_contact=yes
    rtp_symmetric=yes
    ...
    [${linphoneFallbackEndpoint}]
    type=aor
    contact=sip:${linphoneFallback.domain}
    qualify_frequency=60
    ...
    transport=${linphoneFallbackTransport}
    outbound_auth=${linphoneFallbackEndpoint}
    server_uri=sip:${linphoneFallback.domain}
    client_uri=sip:${linphoneFallback.username}@${linphoneFallback.domain}
    contact_user=${linphoneFallback.username}
    retry_interval=60
    forbidden_retry_interval=600
    fatal_retry_interval=600
    expiration=${toString linphoneFallback.registrationExpiry}
    line=yes
    endpoint=${linphoneFallbackEndpoint}
  '';
  ...
  pjsipConf = ''
    ...
    ${natBlockWithLocalNets}
    ${udp6Transport}
    ${outboundTlsTransport}
    ${endpointSections}
    ${trunkSections}
    ${linphoneFallbackSections}
  ...
  # patterns; an empty allow list means "anything not denied" (open). A non-empty
  # allow list is default-deny for everything else -- the toll-fraud control.
  outboundEntry = cfg.dialplan.outboundContext;
  outboundFilter = "${cfg.dialplan.outboundContext}-filter";
  outboundTrunkCtx = "${cfg.dialplan.outboundContext}-trunk";
  outboundReject = "${cfg.dialplan.outboundContext}-reject";

  outboundEmergency = cfg.dialplan.outbound.emergencyNumbers;
  outboundDeny = cfg.dialplan.outbound.denyPatterns;
  ...
  # Open mode (no allow/deny configured): single context, previous behaviour.
  outboundOpenSection = ''
    [${outboundEntry}]
    exten => _X.,1,NoOp(outbound ''${CALLERID(num)} -> ''${EXTEN})
    ${recordingStep}
  ...
  outboundFilteredSection = ''
    ; Entry: emergencies straight to the trunk, everything else through the filter.
    [${outboundEntry}]
    ${concatMapStringsSep "\n" (exactGoto outboundTrunkCtx) outboundEmergency}
    exten => _X.,1,Goto(${outboundFilter},''${EXTEN},1)

    [${outboundFilter}]
    ; Denies first -- belt-and-braces even when an allow pattern would also match.
    ...
    [${outboundReject}]
    exten => _X.,1,NoOp(outbound REJECTED ''${CALLERID(num)} -> ''${EXTEN})
     same => n,Congestion()
     same => n,Hangup()
  '';
  ...
  extensionsConf = ''
    ...
    ${concatStringsSep "\n" (
      mapAttrsToList (num: ext: ''
        ...
        ${optionalString (linphoneFallback.enable && linphoneFallback.targets ? ${num}) ''
          same => n,GotoIf($["''${HANGUPCAUSE}" = "21"]?local-call-rejected)
          same => n,GotoIf($["''${DIALSTATUS}" = "NOANSWER"]?linphone-fallback)
    ...
    ${optionalString linphoneFallback.enable ''
      [${linphoneFallback.inboundContext}]
      ; The Asterisk-owned service account originates fallback calls only.
      exten => ${linphoneFallback.username},1,Hangup(21)
      exten => s,1,Hangup(21)
    ''}
    ...
    ${optionalString trunk.enable ''
      ...
      ; trunk code. Short codes (112/999) fall through unchanged and are handled
      ; as emergencies in the outbound entry context when filtering is on.
      exten => _00X.,1,Goto(${outboundEntry},''${EXTEN:2},1)
      exten => _+X.,1,Goto(${outboundEntry},''${EXTEN:1},1)
      exten => _0X.,1,Goto(${outboundEntry},${trunk.nationalPrefix}''${EXTEN:1},1)
      exten => _X.,1,Goto(${outboundEntry},''${EXTEN},1)

      ${if outboundFilterEnabled then outboundFilteredSection else outboundOpenSection}
  ...
  runtimeConfScript = pkgs.writeShellScript "asterisk-runtime-conf" ''
    ...
    ${optionalString linphoneFallback.enable "emit_auth ${linphoneFallbackEndpoint} ${linphoneFallback.username} ${linphoneFallback.passwordFile}"}
...
in
{
  options.smind.services.asterisk = {
    ...
    trunk = {
      ...
      tlsSourcePort = mkOption {
        ...
        default = 5062;
        description = ''
          Local port the shared client-side TLS transport binds to. Relevant
          when the trunk or Linphone fallback uses TLS. Deliberately not opened
          in the firewall: it originates connections, it does not serve them.
        '';
      };
    ...
    linphoneFallback = {
      ...
      domain = mkOption {
        ...
        default = "sip.linphone.org";
        description = "SIP service domain for the fallback account and destinations.";
      };
      ...
      transport = mkOption {
        ...
        default = "tls";
        description = "SIP transport used for fallback registration and calls.";
      };

      username = mkOption {
        type = types.strMatching "[A-Za-z0-9._+~-]+";
        description = "Username of the SIP account registered by Asterisk.";
      };

      passwordFile = mkOption {
        type = types.path;
        description = "File containing the Asterisk-owned SIP account password.";
      };

      targets = mkOption {
        type = types.attrsOf (types.strMatching "[A-Za-z0-9._+~-]+");
        default = { };
        example = {
          "101" = "mobile-101";
          "102" = "mobile-102";
        ...
        description = ''
          Local extension to destination SIP username mapping. Destination
          accounts stay registered only on their Linphone clients; Asterisk
          registers the separate account configured by `username`.
        '';
      };
      ...
      ringTimeout = mkOption {
        type = types.ints.positive;
        default = 45;
        description = "Seconds to ring the fallback destination.";
      };
      ...
      inboundContext = mkOption {
        type = types.str;
        default = "from-linphone-fallback";
        description = "Isolated context for calls to the Asterisk-owned fallback account.";
      };
    };
    ...
    dialplan = {
      ...
      ringGroups = mkOption {
        type = types.attrsOf (types.nonEmptyListOf types.str);
        default = { };
        description = "Virtual extensions mapped to registered endpoints rung simultaneously.";
      };
      ...
      outbound = {
        emergencyNumbers = mkOption {
          ...
          description = ''
            ...
            numbers are reachable even when allowPatterns is restrictive.
          '';
        };

        denyPatterns = mkOption {
          type = types.listOf types.str;
          # Leading '_' is added when emitting extensions.conf.
          default = [
            # IE premium rate (dialled as 15xx..., no national 0).
            "15X."
            "35315X."
            # IE nomadic VoIP (076) -- not a landline/mobile.
            "35376X."
            # UK premium / high-cost non-geo.
            "449X."
            "4484X."
            "4487X."
            # UK personal numbering (often expensive).
            "4470X."
            # Satellite / global networks commonly abused for toll fraud.
            "870X."
            "881X."
            "882X."
            "883X."
          ...
          description = ''
            Asterisk dialplan patterns (without the leading `_`) rejected before
            any allow pattern is considered. Numbers are already normalised to
            E.164 without '+'.
          '';
        };
        ...
        allowPatterns = mkOption {
          type = types.listOf types.str;
          # Default: IE + UK geographic landlines and mobiles only. Empty means
          # "allow anything not denied" (deny-only mode). Non-empty is an
          # allowlist: anything that matches no allow pattern is rejected.
          default = [
            # IE geographic landlines (01, 02x, 04x-07x, 09x). 03 unused; 08
            # is mobile/non-geo (below). 076 nomadic is in denyPatterns.
            "3531X."
            "3532X."
            "3534X."
            "3535X."
            ...
            "3537X."
            "3539X."
            # IE mobiles: 083/085/086/087/088/089. Excludes 0800/081x non-geo.
            "3538[356789]X."
            # UK geographic (01, 02) and UK-wide landline-rate (03).
            "441X."
            "442X."
            "443X."
            # UK mobiles 071-079; 070 personal is in denyPatterns.
            "447[1-9]X."
          ];
          description = ''
            Asterisk dialplan patterns (without the leading `_`) permitted to
            reach the trunk after denyPatterns. Non-empty => default-deny for
            anything unmatched. Empty => anything not denied is allowed.
            Numbers are already normalised to E.164 without '+'.
          '';
        };
      };
    };
  ...
  config = mkIf cfg.enable {
    ...
    systemd.services.asterisk = {
      ...
      # uses AF_NETLINK for interface discovery. Directives that have broken RTP
      # or the port bind in practice stay off: RestrictRealtime (media timing),
      # SystemCallFilter, CapabilityBoundingSet (root->asterisk drop + bind),
      # MemoryDenyWriteExecute (codec paths). Tighten further only after an
      # on-box register+call test.
      # ProtectSystem=full: /usr /boot /etc read-only; /var and /run writable.
      serviceConfig = {
    ...
    assertions = [
      ...
      {
        assertion = !linphoneFallback.enable || linphoneFallback.targets != { };
        message = "smind.services.asterisk.linphoneFallback requires at least one target";
      }
      {
        assertion =
          !linphoneFallback.enable
          || lib.all (n: cfg.extensions ? ${n}) (lib.attrNames linphoneFallback.targets);
        message = "smind.services.asterisk.linphoneFallback.targets names an undefined local extension";
      }
      {
        assertion = !linphoneFallback.enable || !(cfg.extensions ? ${linphoneFallbackEndpoint});
        message = "smind.services.asterisk extensions must not use the reserved name linphone-fallback";
      }
      {
