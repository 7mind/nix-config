...
in
{
  ...
  config = lib.mkIf config.smind.iperf.enable {
    ...
    environment.systemPackages = with pkgs; [
      ...
      #     '';
      #   })
    ] ++ (if (config.smind.with-private && config.smind.iperf.protected.client.enable) then [
    ...
    services.iperf3 = {
      enable = true;
      openFirewall = true;
      rsaPrivateKey = lib.mkIf (config.smind.with-private && config.smind.iperf.protected.server.enable) config.age.secrets.iperf-private-key.path;
      authorizedUsersFile = lib.mkIf config.smind.iperf.protected.server.enable "/run/iperf-creds";
    };

    system.activationScripts."iperf-password" = lib.mkIf (config.smind.with-private && config.smind.iperf.protected.server.enable)
      ''
