...
#
# MT7927 is NOT supported by mainline as of linux 7.1.x: the in-tree mt7925
...
#
# This derivation mirrors jetm/mediatek-mt7927-dkms v2.13-1: it takes the mt76
# and Bluetooth source from the pristine linux-7.1.3 tarball, applies the
# vendored GPL-2.0 patch set and custom Kbuild, then builds the modules against
# the *running* kernel's headers.
# The result installs into `.../updates/`, which
...
let
  ...
  # match jetm's v2.13-1 `_mt76_kver`; bump in lockstep with the patch set.
  mt76Kver = "7.1.3";
...
stdenv.mkDerivation {
  ...
  version = "2.13-${kernel.version}";

  src = ./.;

  nativeBuildInputs = kernel.moduleBuildDependencies;

  # The patches are context diffs against linux-7.1.3; -p1 from the respective
