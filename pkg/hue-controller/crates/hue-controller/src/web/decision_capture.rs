...
/// info-level events from `hue_controller::*` into the thread-local
/// buffer.
...
impl<S: Subscriber> Layer<S> for DecisionCaptureLayer {
    fn on_event(&self, event: &tracing::Event<'_>, _ctx: Context<'_, S>) {
        ...
        if !target.starts_with("hue_controller") {
            return;
        }
...
mod tests {
    ...
    fn captures_info_events_when_active() {
        ...
        tracing::info!(target: "hue_controller::controller", "should be ignored");
        let before = drain_capture();
        assert!(before.is_empty());

        // Start capturing.
        start_capture();
        tracing::info!(target: "hue_controller::controller", room = "kitchen", "motion on");
        tracing::info!(target: "hue_controller::controller", "scene recall");
        tracing::debug!(target: "hue_controller::controller", "debug is filtered");
    ...
    fn drain_stops_capturing() {
        ...
        start_capture();
        tracing::info!(target: "hue_controller::controller", "first");
        let _ = drain_capture();

        // After drain, capturing is off.
        tracing::info!(target: "hue_controller::controller", "second");
        let captured = drain_capture();
