...
impl Controller {
    /// Execute single-press action rules triggered by a switch press.
    ...
    /// Execute only double-tap action rules for a switch press.
    pub(super) fn dispatch_switch_actions_double_only(
        &mut self,
        device: &str,
        action: SwitchAction,
        ts: Instant,
