...
struct llama_vocab * llama_load_vocab_from_file(const char * fname) {
    ...
    try {
        ...
        struct gguf_context * gguf_ctx = gguf_init_from_file(fname, gguf_params);
        if (!gguf_ctx) {
            LLAMA_LOG_ERROR("%s: failed to load gguf from %s\n", __func__, fname);
            delete vocab;
            return nullptr;
        }
        FILE * file = ggml_fopen(fname, "rb");
        llama_model_loader ml(gguf_ctx, /*set_tensor_data*/nullptr, /*ud*/nullptr,
                              std::string(fname), splits, file,
                              /*use_mmap*/false, /*use_direct_io*/false,
                              /*check_tensors*/false, /*no_alloc*/true,
                              /*kv_overrides*/nullptr, /*tensor_buft_overrides*/nullptr);
        vocab->load(ml, kv);
        if (file) fclose(file);
        gguf_free(gguf_ctx);
    } catch (const std::exception & err) {
