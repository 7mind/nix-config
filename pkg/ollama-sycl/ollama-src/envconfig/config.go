...
var (
	...
	// AllowExternalLibraryPath — local compatibility shim for ollama-sycl's
	// engine kept at llama.cpp@073bb2c20 (older than v0.24.0's native
	// vendored ec98e2002). In 0.23.0 envconfig this flag toggled whether
	// `OLLAMA_LIBRARY_PATH` entries outside the install tree were honoured;
	// v0.24.0 inlined the filtering directly into ggml.go and dropped the
	// flag. Our older engine's ml/backend/ggml/ggml/src/ggml.go still calls
	// `envconfig.AllowExternalLibraryPath()` at two sites — re-export it
	// here so the build links. Behaviour matches the 0.23.0 default: if
	// OLLAMA_ALLOW_EXTERNAL_LIBRARY_PATH is set true, external paths are
	// allowed; otherwise rejected. Drop this shim once we bump the engine
	// past the point where the symbol use was removed.
